...
(define (merge comp list1 list2)
    ...
    (cond ((null? list1) list2)
          ((null? list2) list1)
          ((equal? (car list1) (car list2)) (cons (car list1) (merge comp (cdr list1) (cdr list2))))
...
(define (list-partitions total max-pieces max-value)
  (cond ((= total 0) '(()))
        ...
        ((< total 0) '())
        (else (append 
                (add-to-all max-value (list-partitions (- total max-value) (- max-pieces 1) max-value)) 
                (list-partitions total max-pieces (- max-value 1))))))
    

(define (add-to-all num lists)
  (cond ((null? lists) '())
        (else (cons (append (car lists) (list num)) (add-to-all num (cdr lists))))))
...
(define (flatten lists) 
  (cond ((null? lists) '())
        ((not (list? lists)) (list lists))
        (else (append (flatten (car lists)) (flatten (cdr lists))))))

(define (tree-sums tree) 
  (cond ((equal? (children tree) nil) (list (entry tree)))
        (else (map (lambda (child) (+ (entry tree) child))  (flatten (map tree-sums (children tree)))))))
