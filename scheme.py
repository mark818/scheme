...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        dynamic_frame = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, dynamic_frame)
    else:
...
def do_mu_form(vals):
    ...
    if len(vals.second) > 1:
        vals = Pair('begin', vals.second)
        return MuProcedure(formals, vals)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if not scheme_symbolp(symbol):
            raise SchemeError
        action = vals.second
        env.define(symbol, do_lambda_form(Pair(formals, action),env))
        return symbol
    else:
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            args = rest.map(lambda op: scheme_optimized_eval(op, env))
